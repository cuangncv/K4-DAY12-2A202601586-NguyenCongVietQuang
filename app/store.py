...
class ChatStore:
    ...
    def ping(self) -> bool:
        ...
        try:
            ...
            # Bắt rộng có chủ đích: mất mạng, sai mật khẩu, Redis chưa lên...
            # đều dẫn tới cùng một kết luận "chưa sẵn sàng". Để exception
            # thoát ra thì /readyz trả 500 thay vì 503, và load balancer
            # không hiểu đó là tín hiệu rút instance khỏi cụm.
            return False
    ...
    def add_turn(self, client_id: str, role: str, content: str) -> None:
        ...
        # Chỉ giữ N message gần nhất: prompt phình vô hạn thì tiền token cũng vậy
    ...
    def history(self, client_id: str) -> list[dict]:
        ...
        return [json.loads(item) for item in raw]  # rỗng → list rỗng
